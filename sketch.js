const Engine = Matter.Engine;
const Render = Matter.Render;
const World = Matter.World;
const Bodies = Matter.Bodies;
const Constraint = Matter.Constraint;
const Body = Matter.Body;
const Composites = Matter.Composites;
const Composite = Matter.Composite;

let engine;
let world;
var rope,fruit,ground;
var fruit_con;
var fruit_con_2;

var bg_img;
var food;
var rabbit;

var button;
var bunny;
var piscando,comendo,sad;

function preload()
{
  bg_img = loadImage('background.png');
  food = loadImage('melon.png');
  rabbit = loadImage('Rabbit-01.png');;
  piscando = loadAnimation("blink_1.png","blink_1.png","blink_1.png","blink_1.png","blink_1.png","blink_1.png","blink_1.png","blink_2.png","blink_3.png","blink_2.png");
  comendo = loadAnimation("eat_0.png" , "eat_1.png","eat_2.png","eat_3.png","eat_4.png");
  sad = loadAnimation("sad_2.png","sad_3.png");
  
  piscando.playing = true;
  comendo.playing = true;
  sad.playing = true;
  sad.looping= false;
  comendo.looping = false;

  piscando.frameDelay = 9
  comendo.frameDelay = 20
  sad.frameDelay = 10
}

function setup() {
  createCanvas(500,700);
  frameRate(80);

  engine = Engine.create();
  world = engine.world;
  
  button = createImg('cut_btn.png');
  button.position(220,30);
  button.size(50,50);
  button.mouseClicked(drop);
  


  bunny = createSprite(300,620,100,100);
  bunny.scale = 0.2;
  

  bunny.addAnimation('piscando',piscando);
  bunny.addAnimation('comendo',comendo);
  bunny.addAnimation('crying',sad);
  bunny.changeAnimation('piscando');
  
  rope = new Rope(7,{x:245,y:30});
  ground = new Ground(200,690,600,20);
  
  fruit = Bodies.circle(300,300,20);
  Matter.Composite.add(rope.body,fruit);

  fruit_con = new Link(rope,fruit);

  rectMode(CENTER);
  ellipseMode(RADIUS);
  imageMode(CENTER);
  
}

function draw(){
  background(51);
  image(bg_img,width/2,height/2,490,690);

  if(fruit !=null){
  image(food,fruit.position.x,fruit.position.y,70,70);
  }

  if(Collision(fruit,bunny)){
    bunny.changeAnimation('comendo',comendo);
  }

  if(Collision(fruit,ground.body)){
    bunny.changeAnimation('crying',sad);
  }

  rope.show();
  Engine.update(engine);
  ground.show();


  drawSprites();
}

function drop()
{
  rope.break();
  fruit_con.detach();
  fruit_con = null;
}

function Collision(body,sprite){

  if(body != null){
   let d = dist(body.position.x,body.position.y,sprite.position.x,sprite.position.y)
   if(d <= 50){
     World.remove(world,fruit)
     fruit = null;
     return true;
   }

   else{
     return false;
   }
  }
  

}



